...
def alphabeta( player, m, board, alpha, beta, best_move ):
    ...
    if one_step_win( 1-player, board ) or win(1-player, board):   
    ...
    for r in range( 1, 10):
        if board[r] == EMPTY:         # move is legal
            ...
            this_eval = -alphabeta(1-player,m+1,boards[this_move],-beta,-alpha,best_move)
...
def play():
    ...
    for board in boards[1:]:
        if list(board).count(WE) + list(board).count(OPP) <= 2:
            print(board)
            state = 1
